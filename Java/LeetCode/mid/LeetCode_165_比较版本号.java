 ...
 * 每个修订号由 多位数字 组成，可能包含前导零 。每个版本号至少包含一个字符。
...
public class LeetCode_165_比较版本号 {
    public int compareVersion(String version1, String version2) {
        ...
        int minLen = Math.min(str1s.length, str2s.length);
        for (int i = 0; i < minLen; i++) {
            if (Integer.parseInt(str1s[i]) > Integer.parseInt(str2s[i])) {
                return 1;
            } else if (Integer.parseInt(str1s[i]) < Integer.parseInt(str2s[i])) {
                return -1;
            }
        }
        if (str1s.length > str2s.length) {
            for (int i = minLen; i < str1s.length; i++) {
                if (Integer.parseInt(str1s[i]) != 0) {
                    return 1;
                }
            }
        } else if (str1s.length < str2s.length) {
            for (int i = minLen; i < str2s.length; i++) {
